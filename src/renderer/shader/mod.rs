...
pub trait Shader {
    ...
    fn fragment_shader(&self, input: &[f32]) -> Vec<f32>;
}

// Representa una fuente de luz en la escena
pub struct Light {
    direction: [f32; 3],    // Dirección de la luz (para luces direccionales)
    position: [f32; 3],     // Posición (para luces puntuales)
    color: [f32; 3],        // Color de la luz
    intensity: f32,         // Intensidad global de la luz
    attenuation: [f32; 3],  // Factores de atenuación (constante, lineal, cuadrático)
    cast_shadows: bool,     // Si esta luz genera sombras
    light_type: LightType,  // Tipo de luz
}
...
pub struct DefaultShader {
    ...
    lights: Vec<Light>,     // Lista de luces en la escena
    ambient_intensity: f32, // Intensidad de luz ambiental global
    shadow_map: Option<ShadowMap>, // Mapa de sombras (opcional)
    shadow_bias: f32,       // Bias para evitar acné de sombra
    specular_intensity: f32, // Intensidad especular global
    shininess: f32,         // Factor de brillo para materiales
}
...
impl DefaultShader {
    ...
    fn calculate_lighting(&self, position: &[f32; 3], normal: &[f32; 3], view_dir: &[f32; 3]) -> [f32; 3] {
        ...
        for light in &self.lights {
            ...
            let shadow_factor = if light.cast_shadows && self.shadow_map.is_some() {
    ...
    fn calculate_shadow_factor(&self, position: &[f32; 3], light: &Light) -> f32 {
        // Si no hay mapa de sombras, no hay sombra
...
impl Shader for DefaultShader {
    fn vertex_shader(&self, input: &Vertex) -> Vec<f32> {
        ...
        // Posición - usamos la posición original sin modificar
        // para permitir que las transformaciones del pipeline funcionen correctamente
        output.push(input.position[0]);
        output.push(input.position[1]);
        output.push(input.position[2]);
        ...
        output.push(input.normal[0] / normal_len);
        output.push(input.normal[1] / normal_len);
        output.push(input.normal[2] / normal_len);
        
        // Coordenadas de textura
        output.push(input.tex_coords[0]);
        output.push(input.tex_coords[1]);
        
        // Color del vértice
        output.push(input.color[0]);
        output.push(input.color[1]);
        output.push(input.color[2]);
        output.push(input.color[3]);
        
        // Pasamos también la posición mundial para cálculos de iluminación
        // Nota: Esta posición será transformada por el pipeline y actualizada
        output.push(input.position[0]); // Posición mundial X
        output.push(input.position[1]); // Posición mundial Y
        output.push(input.position[2]); // Posición mundial Z
    ...
    fn fragment_shader(&self, input: &[f32]) -> Vec<f32> {
        ...
        let normal = [input[3], input[4], input[5]];
        let tex_coords = [input[6], input[7]];
        let vertex_color = [input[8], input[9], input[10], input[11]];
        let world_position = [input[12], input[13], input[14]];
